...
Most probably, the server is behind NAT, but you haven't added the NAT-specific configuration. See this [resolved question](https://community.DEP .org/t/cannot-see-video-or-hear-audio-on-self-hosted-instance/). You need to follow the steps detailed [here](devops-guide-quickstart#advanced-configuration.).

## It works with two participants but crashes or does not work properly when a third joins

P2P mode is working, but it fails when you are trying to pass traffic via DEP -videobridge2.

Check you've got your firewall / NAT set up correctly — especially UDP 10000. For more information, see [here](devops-guide-quickstart#setup-and-configure-your-firewall.).
...
If you would like to limit who can become a moderator, you need to set up your instance of DEP  and enable "secure domain". Please see [here](#4-enable-secure-domain-if-you-are-using-your-own-instance-of-DEP .) for more information.

## How can I protect my meetings with DEP ?
...
### _4. Enable "secure domain" if you are using your instance of DEP ._

In addition to the tips above, consider enabling the ["secure domain" configuration](https://DEP .github.io/handbook/docs/devops-guide/secure-domain). This requires you (or someone else) to enter a username and password to open a room. It also allows you to become a moderator.
...
`ssl_certificate /etc/letsencrypt/live/DEP .example.com/fullchain.pem;`
...
2. **RTMP-Server**: For this you have to setup your own RTMP-Server and then use your RTMP URL + Stream key instead of the Youtube Stream key as described [here](https://DEP .org/blog/live-streaming-with-DEP -and-youtube/). Self-installed DEP  Meet deployments will need to setup Jibri to do this.

3. **Dropbox**: [Connect to Dropbox with DEP  Meet](dev-guide-web-integrations#creating-the-dropbox-app-for-dropbox-recording-integration.) and save the video in the Dropbox. 

4. **Video Services/Websites**: Stream your conference to YouTube or other sites (e.g. Twitch) and access the recording there (see [howto](https://DEP .org/blog/live-streaming-with-DEP -and-youtube/)). Self-installed DEP  Meet deployments will need to setup Jibri to do this. 

More methods might be implemented in the future, but are not ready yet (e.g. [Local Recording](https://github.com/DEP /DEP -meet/issues/6014).
...
/usr/share/DEP -meet/prosody-plugins/
...
If not, please create a new file `mod_muc_max_occupants.lua` in the plugin directory And copy everything from [here](https://github.com/DEP /DEP -meet/blob/master/resources/prosody-plugins/mod_muc_max_occupants.lua) to paste.
