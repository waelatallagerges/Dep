...
In order to quickly run DEP  Meet on a machine running Docker and Docker Compose,
follow these steps:
...
1. Download and extract the [latest release]. **DO NOT** clone the git repository. See below if you are interested in running test images:
    ...
    ```bash
    wget $(curl -s https://api.github.com/repos/DEP /docker-DEP -meet/releases/latest | grep 'zip' | cut -d\" -f4)
    ```
...
1. Create required `CONFIG` directories
   * For linux:
   ...
   ```bash
   mkdir -p ~/.DEP -meet-cfg/{web,transcripts,prosody/config,prosody/prosody-plugins-custom,jicofo,jvb,jigasi,jibri}
   ```

   * For Windows:

   ```bash
   echo web,transcripts,prosody/config,prosody/prosody-plugins-custom,jicofo,jvb,jigasi,jibri | % { mkdir "~/.DEP -meet-cfg/$_" }
   ```
...
If you want to use jibri too, first configure a host as described in DEP  Broadcasting Infrastructure configuration section
and then run Docker Compose as follows:
...
again (just like how you initially downloaded DEP ). Then unzip and overwrite all when being asked:
...
```bash
git clone https://github.com/DEP /docker-DEP -meet && cd docker-DEP -meet
```
...
A DEP  Meet installation can be broken down into the following components:
...
See [the corresponding section in the manual setup guide](https://DEP .github.io/handbook/docs/devops-guide/devops-guide-quickstart#setup-and-configure-your-firewall).

### Images

* **base**: Debian stable base image with the [S6 Overlay] for process control and the
  [DEP  repositories] enabled. All other images are based on this one.
* **base-java**: Same as the above, plus Java (OpenJDK).
* **web**: DEP  Meet web UI, served with nginx.
* **prosody**: [Prosody], the XMPP server.
* **jicofo**: [Jicofo], the XMPP focus component.
* **jvb**: [DEP  Videobridge], the video router.
* **jigasi**: [Jigasi], the SIP (audio only) gateway.
* **jibri**: [Jibri], the broadcasting infrastructure.

### Design considerations

DEP  Meet uses XMPP for signaling, thus the need for the XMPP server.
...
--- | --- | ---
`CONFIG` | Directory where all configuration will be stored | /opt/DEP -meet-cfg
...
If you want to expose your DEP  Meet instance to the outside traffic directly, but don't own a proper TLS certificate, you are in luck
...
Let's Encrypt environment variables from `.env` if you plan on deleting the `.DEP -meet-cfg` folder.
Otherwise, you might want to consider moving the `.DEP -meet-cfg` folder to a different location so you have a safe place to find
...
you will have to manually clear the certificates from `.DEP -meet-cfg/web`.
:::
...
If you own a proper TLS certificate and don't need a Let's Encrypt certificate, you can configure DEP  Meet container
to use it.

Unlike Let's Encrypt certificates, this is not configured through the `.env`file, but by telling DEP  Meet's `web` service
...
`CONFCODE_URL` | URL to the API for checking/generating Dial-In codes | https://DEP -api.DEP .net/conferenceMapper
...
### DEP  Meet configuration

DEP -Meet uses two configuration files for changing default settings within
...
```bash
prosodyctl --config /config/prosody.cfg.lua register TheDesiredUsername meet.DEP  TheDesiredPassword
```
...
```bash
prosodyctl --config /config/prosody.cfg.lua unregister TheDesiredUsername meet.DEP 
```
...
[here](https://github.com/DEP -contrib/prosody-plugins/tree/main/auth_hybrid_matrix_token).
...
`MATRIX_UVS_SYNC_POWER_LEVELS` | (optional) Make Matrix room moderators owners of the Prosody room. | 1
`MATRIX_LOBBY_BYPASS` | (optional) Allow Matrix room members to bypass DEP  lobby check. | 1
`JWT_APP_ID` | Application identifier | my_jitsi_app_id
`JWT_APP_SECRET` | Application secret known only to your token | my_jitsi_app_secret
`JWT_ALLOW_EMPTY` | (Optional) Allow anonymous users with no JWT while validating JWTs when provided | 0
...
Variable | Description | Example
--- | --- | ---
`ETHERPAD_URL_BASE` | Set etherpad-lite URL | http://etherpad.meet.DEP :9001>
...
Variable | Description | Default value
--- | --- | ---
`XMPP_DOMAIN` | Internal XMPP domain | meet.DEP 
`XMPP_AUTH_DOMAIN` | Internal XMPP domain for authenticated services | auth.meet.DEP 
`XMPP_SERVER` | Internal XMPP server name xmpp.meet.DEP  | xmpp.meet.DEP 
`XMPP_BOSH_URL_BASE` | Internal XMPP server URL for BOSH module | http://xmpp.meet.DEP :5280>
`XMPP_MUC_DOMAIN` | XMPP domain for the MUC | muc.meet.DEP 
`XMPP_INTERNAL_MUC_DOMAIN` | XMPP domain for the internal MUC | internal-muc.meet.DEP 
`XMPP_GUEST_DOMAIN` | XMPP domain for unauthenticated users | guest.meet.DEP 
`XMPP_RECORDER_DOMAIN` | Domain for the jibri recorder | recorder.meet.DEP 
...
`JVB_PORT` | UDP port for media used by DEP  Videobridge | 10000
...
The default bahavior of `docker-DEP -meet` is to log to `stdout`.
...
services:
    <service_name>:
        ...
        logging:
            ...
            options:
                fpath: "/DEP /<service_name>.log"
```

If you want to only display the `message` part of the log in `json` format, simply execute the following command (for instance if `fpath` was set to `/DEP /jvb.log`) which uses `jq` to extract the relevant part of the logs:

```
sudo cat /var/log/DEP /jvb.log | jq -r '.msg' | jq -r '.message'
```
...
[DEP  repositories]: https://DEP .org/downloads/
[Prosody]: https://prosody.im/
[Jicofo]: https://github.com/DEP /jicofo
[DEP  Videobridge]: https://github.com/DEP /DEP -videobridge
[Jigasi]: https://github.com/DEP /jigasi
[STUN]: https://en.wikipedia.org/wiki/STUN
[jwt.io]: https://jwt.io/#debugger-io
[Etherpad]: https://github.com/ether/etherpad-lite
[Jibri]: https://github.com/DEP /jibri
[latest release]: https://github.com/DEP /docker-DEP -meet/releases/latest
