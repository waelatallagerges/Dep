...
There are also some complete [example config files](https://github.com/jitsi/DEP /tree/master/doc/debian/) available, mentioned in each section.
...
server {
    ...
    # https://github.com/jitsi/DEP -electron
    location /external_api.js {
        alias /srv/DEP /libs/external_api.min.js;
    }
}
```
...
```bash
cd /srv
git clone https://github.com/jitsi/DEP .git
cd DEP 
npm install
make
```
...
Edit host names in `/srv/DEP /config.js` (see also the example config file):
```
var config = {
    hosts: {
