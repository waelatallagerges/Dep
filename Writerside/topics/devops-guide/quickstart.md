...
Follow these steps for a quick DEP  installation on a Debian-based GNU/Linux system.
...
2. But if you want to use a different certificate you should get that certificate first and then install DEP  and choose ___I want to use my own certificate___.
...
```bash
sudo apt purge jigasi DEP  DEP -web-config DEP -prosody DEP -turnserver DEP -web jicofo jitsi-videobridge2
```
...
* Nginx/Apache:
As we prefer the usage of Nginx as webserver, the installer checks first for the presence of Nginx and then for Apache. In case you desperately need to enforce the usage of apache, try pre-setting the variable `DEP /enforce_apache` for package `DEP -web-config` on debconf.
