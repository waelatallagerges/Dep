...
You can control the embedded DEP Meet conference by calling **`executeCommand`** on the **`DEPMeetExternalAPI`** object:
...
Starts a local recording, file recording or streaming session using passed parameters:
  ...
  Additionally, Dropbox saving should be enabled on the DEP meet deploy config you are using.
...
api.executeCommand('startRecording', {
    ...
    shouldShare: boolean, //whether the recording should be shared with the participants or not. Only applies to certain DEP meet deploys.
