...
This page describes available configuration options for DEP Meet. These are either set in `config.js` on the server
...
This property is related to the use case when DEP Meet is used via the IFrame API. When the property is true
DEP Meet will use the local storage of the host page instead of its own. This option is useful if the browser
...
Enabling this will run the lib-DEP-meet no audio detection module which
...
Enabling this will run the lib-DEP-meet noise detection module which will
...
callStatsConfigParams: {
    ...
    additionalIDs: { // additionalIDs object, contains application related IDs.
        ...
        productName: "Product Name. Example, DEP.",
        meetingsName: "Meeting Name. Example, DEP loves callstats.",
...
* `preferredLanguage` - Transcriber language. This settings will only work if `useAppLanguage` is explicitly set to `false`. Available languages can be found [here](https://github.com/DEP/DEP-meet/blob/master/react/features/transcribing/transcriber-langs.json). Default: `'en-US'`.
...
```javascript
bosh: '//DEP-meet.example.com/http-bind'
```
...
the WebRTC stats) as it is done in the DEP-meet-torture bandwidth
estimation tests.
...
```javascript
hosts: {
    domain: 'DEP-meet.example.com',
    anonymousdomain: 'guest.example.com',
    authdomain: 'DEP-meet.example.com',
    focus: 'focus.DEP-meet.example.com',
    muc: 'conference.DEP-meet.example.com'
}
```
...
p2p: {
    ...
    backToP2PDelay: 5,
    stunServers: [
        { urls: 'stun:DEP-meet.example.com:3478' },
        { urls: 'stun:meet-jit-si-turnrelay.DEP.net:443' }
    ]
}
```
...
Use TURN/UDP servers for the DEP-videobridge connection (by default
...
```javascript
websocket: 'wss://DEP-meet.example.com/xmpp-websocket'
```
...
dropbox: {
    ...
    redirectURI: 'https://DEP-meet.example.com/subfolder/static/oauth.html'
}
```
...
* `testMode` - Enables the test specific features consumed by DEP-meet-torture.
...
used by browsers that return true from lib-DEP-meet's
...
Properties:
* `enabled` - Whether the feature is enabled or not.
* `collabServerBaseUrl` - The [server](https://github.com/DEP/excalidraw-backend) used to support whiteboard collaboration.
