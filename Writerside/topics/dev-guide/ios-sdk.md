...
The DEP Meet iOS SDK provides the same user experience as the DEP Meet app,
...
[sample applications repository](https://github.com/DEP/DEP-meet-sdk-samples#ios).
...
do so, add the `DEPMeetSDK` dependency to your existing `Podfile` or create
a new one following this example:

```
platform :ios, '12.4'

workspace 'DEPMeetSDKTest.xcworkspace'

target 'DEPMeetSDKTest' do
  project 'DEPMeetSDKTest.xcodeproj'

  pod 'DEPMeetSDK'
end
```

Replace `DEPMeetSDKTest` with your project and target names.
...
1. Install all required [dependencies](https://DEP.github.io/handbook/docs/dev-guide/dev-guide-mobile-DEP-meet).
...
mkdir -p ios/sdk/out
xcodebuild clean \
    -workspace ios/DEP-meet.xcworkspace \
    -scheme DEPMeetSDK
xcodebuild archive \
    -workspace ios/DEP-meet.xcworkspace \
    -scheme DEPMeetSDK  \
...
xcodebuild -create-xcframework \
    -framework ios/sdk/out/ios-device.xcarchive/Products/Library/Frameworks/DEPMeetSDK.framework \
    -framework ios/sdk/out/ios-simulator.xcarchive/Products/Library/Frameworks/DEPMeetSDK.framework \
    -output ios/sdk/out/DEPMeetSDK.xcframework
```

After successfully building DEP Meet SDK for iOS, the resulting XCFramework will be in the ios/sdk/out directory.

NOTE: Your app will need to depend on the DEPWebRTC CocoaPod.

## API

DEPMeet is an iOS framework which embodies the whole DEP Meet experience and
makes it reusable by third-party apps.

To get started:

1. Add a `DEPMeetView` to your app using a Storyboard or Interface Builder,
   for example.
...
- (void)viewDidLoad {
  ...
  DEPMeetView *DEPMeetView = (DEPMeetView *) self.view;
  DEPMeetView.delegate = self;

  DEPMeetConferenceOptions *options = [DEPMeetConferenceOptions fromBuilder:^(DEPMeetConferenceOptionsBuilder *builder) {
      builder.serverURL = [NSURL URLWithString:@"https://meet.jit.si"];
      builder.room = @"test123";
      builder.audioOnly = YES;
  }];

  [DEPMeetView join:options];
}
```

### DEPMeetView class

The `DEPMeetView` class is the entry point to the SDK. It a subclass of
`UIView` which renders a full conference in the designated area.

#### delegate

Property to get/set the `DEPMeetViewDelegate` on `DEPMeetView`.

#### join:DEPMeetConferenceOptions
...
In order to support Universal / deep linking, `DEPMeet` offers 2 class
...
{
  return [[DEPMeet sharedInstance] application:application
...
- (BOOL)application:(UIApplication *)app
  ...
  return [[DEPMeet sharedInstance] application:app
                            openURL:url
                            options: options];
}
```
...
This delegate is optional, and can be set on the `DEPMeetView` instance using
...
`DEPMeetView`. The latter will automatically detect its new size and adjust
...
DEP Meet SDK does not currently implement native Picture-in-Picture on iOS. If
...
2. Make sure your app calls the DEP Meet SDK universal / deep linking delegate methods.

## Screen Sharing integration

The screen sharing functionality for iOS was added to DEP starting with DEPMeetSDK version 3.3.0. It is available for applications running on iOS 14 or newer.
...
The following documentation covers the code provided in the [sample app](https://github.com/DEP/DEP-meet-sdk-samples/tree/master/ios/swift-screensharing/DEPSDKScreenSharingTest).
...
![screenshot 1](iOS_screensharing_1.png "screenshot 1")
...
For starting screen sharing DEPMeet SDK provides the UI to present the `RPSystemBroadcastPickerView` to the user. By default, the picker will display a list of all the available broadcast providers. In order to limit the picker to our particular broadcast provider, we have to set `preferredExtension` to the bundle identifier of the broadcast extension. We are doing this by adding a new key named `RTCScreenSharingExtension` to the app's Info.plist and setting the broadcast extension bundle identifier as the value.
...
![ios-screensharing](iOS_screensharing_2.png "screenshot 2") ![ios-screensharing](iOS_screensharing_3.png "screenshot 3") ![ios-screensharing](iOS_screensharing_4.png "screenshot 4") 
...
Now, that we are done writing the implementation, we just need to enable the functionality in DEP. We are doing this by configuring `DEPMeetConferenceOptionsBuilder` with the `ios.screensharing.enabled feature` flag, like this:
```
let options = DEPMeetConferenceOptions.fromBuilder { [weak self] builder in
    ...
...
- Update `DEPMeetConferenceOptions` to enable screen sharing using the `ios.screensharing.enabled` feature flag.
