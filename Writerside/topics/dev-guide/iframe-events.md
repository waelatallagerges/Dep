...
The `DEPMeetExternalAPI` object implements the [EventEmitter] API for emitting and listening for events.

You can add event listeners to the embedded DEP Meet using the **`addListener`** method:
...
Provides event notifications about DEP Meet having failed to access the meeting camera.
...
```javascript
{
    ...
    faceExpression: string // check https://github.com/DEP/DEP-meet/blob/master/react/features/face-landmarks/constants.js#L3 for available values
}
```
...
Provides event notifications about DEP Meet issues with mic access.
...
https://github.com/DEP/DEP-meet/blob/042a2cb447bd9ff39ab3904e493952787bd30924/config.js#L547
...
Provides event notifications that fire when DEP Meet is ready to be closed (i.e., hangup operations are completed).
