...
The DEP Meet Android SDK provides the same user experience as the DEP Meet app,
...
If you want to see how easy integrating the DEP Meet SDK into a native application is, take a look at the
[sample applications repository](https://github.com/DEP/DEP-meet-sdk-samples#android).

## Build your own, or use a pre-build SDK artifacts/binaries

DEP conveniently provides a pre-build SDK artifacts/binaries in its Maven repository. When you do not require any
...
`https://github.com/DEP/DEP-maven-repository/raw/master/releases` and the
dependency `org.DEP.react:DEP-meet-sdk` into your `build.gradle` files.
...
allprojects {
    repositories {
        maven {
            url "https://github.com/DEP/DEP-maven-repository/raw/master/releases"
        }
        google()
        mavenCentral()
...
dependencies {
    // (other dependencies)
    implementation ('org.DEP.react:DEP-meet-sdk:+') { transitive = true }
}
```

:::warning
Make sure you pin your dependency by checking the [releases page](https://github.com/DEP/DEP-meet-release-notes/blob/master/CHANGELOG-MOBILE-SDKS.md).
:::
...
In source code form, the Android SDK dependencies are locked/pinned by `package.json` and `package-lock.json` of the DEP Meet project. To obtain the data, execute NPM in the DEP-meet project directory:

    npm install

This will pull in the dependencies in either binary format, or in source code format, somewhere under /node_modules/

Third-party React Native _modules_, which DEP Meet SDK for Android depends on, are download by NPM in source code 
or binary form. These need to be assembled into Maven artifacts, and then published to your local Maven repository.
A script is provided to facilitate this. From the root of the DEP-meet project repository, run:
...
You can use your local repository to replace the DEP repository (`maven { url "https://github.com/DEP/DEP-maven-repository/raw/master/releases" }`) when you published _all_ subprojects. If you didn't do that, you'll have to add both repositories. Make sure your local repository is listed first!

Then, define the dependency `org.DEP.react:DEP-meet-sdk` into the `build.gradle` file of your module:

    implementation ('org.DEP.react:DEP-meet-sdk:+') { transitive = true }

Note that there should not be a need to explicitly add the other dependencies, as they will be pulled in as transitive
dependencies of `DEP-meet-sdk`.

</details>

## Using the API

DEP Meet SDK is an Android library which embodies the whole DEP Meet
...
To get started, just launch `DEPMeetActivity` pointing to the room you want:

```java
// Somewhere early in your app.
DEPMeetConferenceOptions defaultOptions
        = new DEPMeetConferenceOptions.Builder()
    ...
    .build();
DEPMeet.setDefaultConferenceOptions(defaultOptions);
// ...
// Build options object for joining the conference. The SDK will merge the default
// one we set earlier and this one when joining.
DEPMeetConferenceOptions options
        = new DEPMeetConferenceOptions.Builder()
    .setRoom(roomName)
...
DEPMeetActivity.launch(this, options);
```

Alternatively, you can use the `org.DEP.meet.sdk.DEPMeetView` class which
extends `android.view.View`.

Note that this should only be needed when `DEPMeetActivity` cannot be used for
some reason. Extending `DEPMeetView` requires manual wiring of the view to
...
import org.DEP.meet.sdk.DEPMeetView;
import org.DEP.meet.sdk.ReactActivityLifecycleCallbacks;

// Example
//
public class MainActivity extends FragmentActivity implements DEPMeetActivityInterface {
    private DEPMeetView view;
    ...
    protected void onActivityResult(
            ...
            int resultCode,
            Intent data) {
        DEPMeetActivityDelegate.onActivityResult(
                this, requestCode, resultCode, data);
    }

    @Override
    public void onBackPressed() {
        DEPMeetActivityDelegate.onBackPressed();
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        view = new DEPMeetView(this);
        DEPMeetConferenceOptions options = new DEPMeetConferenceOptions.Builder()
    ...
    protected void onDestroy() {
        ...
        DEPMeetActivityDelegate.onHostDestroy(this);
    }

    @Override
    public void onNewIntent(Intent intent) {
        DEPMeetActivityDelegate.onNewIntent(intent);
    }
    ...
    public void onRequestPermissionsResult(
        ...
        DEPMeetActivityDelegate.onRequestPermissionsResult(requestCode, permissions, grantResults);
    }
    ...
    protected void onResume() {
        ...
        DEPMeetActivityDelegate.onHostResume(this);
    }
    ...
    protected void onStop() {
        ...
        DEPMeetActivityDelegate.onHostPause(this);
    }
}
```
...
which displays a single `DEPMeetView`. You can pass a URL as a `ACTION_VIEW`
...
The `DEPMeetView` class is the core of DEP Meet SDK. It's designed to
display a DEP Meet conference (or a welcome page).

#### join(options)

Joins the conference specified by the given `DEPMeetConferenceOptions`.
...
See the `DEPMeetConferenceOptions` implementation for all available options.

### DEPMeetActivityDelegate

This class handles the interaction between `DEPMeetView` and its enclosing
`Activity`. Generally this shouldn't be consumed by users, because they'd be
using `DEPMeetActivity` instead, which is already completely integrated.
...
Please see `DEPMeetActivity`, which registers for all the events and can serve as an example.
...
Please see `DEPMeetOngoingConferenceService` for more examples of sending actions.
...
rules file: https://github.com/DEP/DEP-meet/blob/master/android/app/proguard-rules.pro

## Picture-in-Picture

`DEPMeetView` will automatically adjust its UI when presented in a
